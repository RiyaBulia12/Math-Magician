...
describe('Operate: Negative test cases', () => {
  const one = -3;
   const two = 0
    it('should error when divide with 0', () => {
      expect(operate(one, two, '÷')).toEqual("Can't divide by 0.");
   });

    it('should error when modulo with 0', () => {
      expect(operate(one, two, '%')).toEqual("Can't find modulo as can't divide by 0.");
   });

   it('should error when operation is unknown', () => {
    expect(() => {operate(two, 0, '/')}).toThrow(Error(`Unknown operation '/'`));
   });
